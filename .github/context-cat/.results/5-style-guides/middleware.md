...
- **Global Origin Verification Mounting**: The application globally registers a custom `verifyGatewayOrigin` middleware in `server.ts` to block any direct HTTP access bypassing the API gateway for all endpoints, except internal container checks like `/health` and `/api/health`.
