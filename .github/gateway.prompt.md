...
{final_output_file} = /.github/context-gate/gateway.md
...
6. All output must be in `/.github/context-gate` folder, For each step, output results into a corresponding `.github/context-gate/{output_folder}/` folder.
    - Mirror the step’s filename e.g., `1-determine-techstack.md` > `.github/context-gate/{output_folder}/1-determine-techstack.md`.
Stop ONLY when:
    - All `instruction-generation` steps are complete
    - A full `{final_output_file}` can be generated.
7. if you're unable to fetch the github repo, make reference to the files in {target} to get an idea of {target}.