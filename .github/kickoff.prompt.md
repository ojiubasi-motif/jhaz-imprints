...
You are assisting with generating a {final_output_file} file using a multi-step prompt chain. the target application for this generation is `packages` directory of this repository.
...
6. All output must be in `/.github/context-api` folder, For each step, output results into a corresponding `.github/context-api/{output_folder}/` folder.
    - Mirror the step’s filename e.g., `1-determine-techstack.md` > `.github/context-api/{output_folder}/1-determine-techstack.md`.
Stop ONLY when:
