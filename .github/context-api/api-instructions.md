...
- **Key Conventions**:
  ...
  - Use Prisma transactions for atomic updates in PostgreSQL.
  - Queries MongoDB collections directly at runtime via Mongoose models (`Product` and `Fabric` from `@jhaz-imprints/catalog-db`) to fetch source-of-truth catalogs during order creation/validation.
...
- **Key Conventions**:
  - Enforce strict idempotency using unique reference IDs (e.g. `order_<id>_<timestamp>`).
  - Expose `POST /:orderId/payment-intent` for initializing a Paystack transaction which delegates to `orderService.initializeOrderPayment`.
  ...
  - The `createOrder` service directly initializes Paystack and returns initial payment configurations (`paymentUrl`, `accessCode`, `reference`, `breakdown`).
...
- **Database**:
  ...
  - Catalog/Product and Fabric data is read directly from MongoDB (via Mongoose models `Product` and `Fabric` in `@jhaz-imprints/catalog-db`) at runtime during transactions (e.g. order creation).
  - The PostgreSQL `CachedProduct` table is maintained as a replica via the Redis Stream replication worker (`catalogEventWorker`) but is not directly read during checkout.
- **Order Snapshotting**: When creating an order, ALWAYS save snapshots of the product details (name, selected options, base price, modifiers, measurements, and notes) inline in the `items` `Json` column of the Prisma `Order` table. This ensures historical accuracy even if the product catalog changes later.
...
- **Order Responses**: When returning orders (GET `/orders/:id` or `/orders/my-orders`), the service fetches the order from Prisma directly. No runtime database joins or catalog cache enrichments are needed, since all product snapshots and customization choices are stored inline in the order's `items` array.
- **Validation & Defensive Programming**: 
  - **Always validate inputs** using `@jhaz-imprints/shared` Zod schemas. 
  - **Strict String Matching**: When comparing frontend payloads with database strings (especially product/fabric options), ALWAYS use `.trim().toLowerCase()` to prevent invisible character mismatch errors.
