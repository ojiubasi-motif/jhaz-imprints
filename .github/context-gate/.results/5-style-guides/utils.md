...
- **Header Stripping & Sanitization**: Explicitly deletes specific header keys (like `host`, `content-length`, and `authorization`) before forwarding to downstream targets. Stripping `authorization` prevents downstream JWT re-verification.
