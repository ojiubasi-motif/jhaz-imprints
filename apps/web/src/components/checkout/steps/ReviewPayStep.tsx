...
export default function ReviewPayStep() {
  ...
  const handlePayNow = async () => {
    ...
    // Clean up formData to prevent 400 Bad Request
    const cleanedData: any = { ...formData };
    ["chest", "waist", "hip", "shoulder", "armLength", "length"].forEach((key) => {
      if (isNaN(cleanedData[key]) || cleanedData[key] === null || cleanedData[key] === "") {
    ...
    try {
      ...
      const resultAction = await dispatch(createOrder(cleanedData as OrderCreate));

      if (!createOrder.fulfilled.match(resultAction)) {
        throw new Error("Failed to create order");
      }

      const { orderId, reference, paystackAccessCode, totalAmount } = resultAction.payload;

      // Step 3: Open Paystack modal (Step 4 in prompt)
      ...
      const handler = window.PaystackPop.setup({
        ...
        email: user?.email,
        amount: (totalAmount || total) * 100, // Use backend amount with frontend fallback, convert to kobo
        ref: reference,
        ...
        callback: function(response: any) {
          ...
          // Move async logic inside standard function
          (async () => {
            dispatch(clearDraft());
            try {
              ...
              const successUrl = `/orders?status=success&orderId=${orderId}`;
              router.push(successUrl);
              setTimeout(() => { window.location.href = successUrl; }, 1000);
            } catch (error) {
              console.error("Manual verification failed:", error);
              router.push(`/orders?status=verifying&orderId=${orderId}`);
            }
          })();
        },
      ...
      setPaymentError(error.message || "An unexpected error occurred during payment. Please try again.");
      setIsProcessingPayment(false);
    }
  };
  ...
  const handlePayLater = async () => {
    ...
    try {
      ...
      if (createOrder.fulfilled.match(resultAction)) {
        dispatch(clearDraft());
        router.push(
          `/orders/${resultAction.payload.orderId}?status=offline-pending&message=payment-deferred`
        );
      }
    } catch (error) {
